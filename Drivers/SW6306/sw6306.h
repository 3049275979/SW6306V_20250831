...
#define SW6306_PD_20V_FIX_CURR          4500U       //PD 20V Fix电流（必须是10的倍数）
...
#define SW6306_PD_PPS3_CURR             4500U       //PD PPS3电流（最大21V）（必须是50的倍数）
...
#define SW6306_CTRG_LED_DISPLAY             0x4BU//灯显控制
...
#define SW6306_DISPLAY_NUM_MSK		0x0FU//灯显控制寄存器有效位
...
#define SW6306_DISPLAY_CUR_MSK          0x0EU//显示设置寄存器有效位
...
uint8_t SW6306_RegsetSwitch(uint16_t regset);//寄存器组切换
uint8_t SW6306_ByteModify(uint8_t reg, uint8_t mask, uint8_t data);//修改对应寄存器的特定位
...
3. 充放电功率设置各有两个寄存器，其中0x100寄存器不使用，使用0x40地址的寄存器设置，以便精确到1W
*/
